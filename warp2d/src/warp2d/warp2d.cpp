...
//{
// auto peaks = Warp2D::peaks_in_rt_range(target_peaks, rt_start, rt_end);
// auto filtered_peaks = Warp2D::filter_peaks(peaks, n_peaks_per_segment);
...
//// Filter source peaks.
//{
// auto peaks = Warp2D::peaks_in_rt_range(source_peaks, rt_start, rt_end);
// auto filtered_peaks = Warp2D::filter_peaks(peaks, n_peaks_per_segment);
...
// Warp2D::peaks_in_rt_range(target_peaks_filtered, rt_start, rt_end);
...
// auto source_peaks_warped = Warp2D::peaks_in_rt_range(
...
// Warp2D::peaks_in_rt_range(source_peaks, sample_rt_start, sample_rt_end);
...
// auto filtered_peaks =
// Warp2D::filter_peaks(peaks, n_peaks_per_segment);
// target_peaks_filtered.insert(end(target_peaks_filtered),
// begin(filtered_peaks),
// end(filtered_peaks));
//}
...
// auto ref_rt_end) {
// auto warped_peaks = Warp2D::peaks_in_rt_range(
// source_peaks, source_rt_start, source_rt_end);
...
// auto compute_warped_similarities = [&perform_warpings](
...
//&source_peaks_filtered, &compute_warped_similarities]() {
// for (const auto& k : groups[i]) {
// auto& current_level = levels[k];
// double rt_start = rt_min + k * segment_rt_width;
// double rt_end = rt_start + segment_rt_width;
// compute_warped_similarities(
