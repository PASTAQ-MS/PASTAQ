...
TEST_CASE("Peak warping using warp2d") {
    ...
    SUBCASE("Parallel execution") {
        {
            auto warped_peaks = Warp2D::Runners::Parallel::run(
                target_peaks, source_peaks, {5, 10, 1000, 50, 0.2}, 1);
        ...
        {
            auto warped_peaks = Warp2D::Runners::Parallel::run(
                target_peaks, source_peaks, {5, 10, 1000, 50, 0.2}, 4);
            bool sizes_match = warped_peaks.size() == source_peaks.size();
            CHECK(sizes_match);
            if (sizes_match) {
                for (size_t i = 0; i < warped_peaks.size(); ++i) {
        ...
        {
            auto warped_peaks = Warp2D::Runners::Parallel::run(
                target_peaks, source_peaks, {5, 10, 1000, 50, 0.2}, 8);
            bool sizes_match = warped_peaks.size() == source_peaks.size();
            CHECK(sizes_match);
            if (sizes_match) {
                for (size_t i = 0; i < warped_peaks.size(); ++i) {
        ...
        {
            auto warped_peaks = Warp2D::Runners::Parallel::run(
                target_peaks, source_peaks, {5, 10, 1000, 50, 0.2}, 12);
            bool sizes_match = warped_peaks.size() == source_peaks.size();
            CHECK(sizes_match);
            if (sizes_match) {
                for (size_t i = 0; i < warped_peaks.size(); ++i) {
    ...
    SUBCASE("Serial execution") {
        auto warped_peaks = Warp2D::Runners::Serial::run(
            target_peaks, source_peaks, {5, 10, 1000, 50, 0.2});
        bool sizes_match = warped_peaks.size() == source_peaks.size();
        CHECK(sizes_match);
        if (sizes_match) {
            for (size_t i = 0; i < warped_peaks.size(); ++i) {
