...
TEST_CASE("Bounds check on RegularMesh when using RegularMesh::at") {
    SUBCASE("An empty mesh should always return std::nullopt") {
        auto mesh = RegularMesh({0, 0}, {0.0, 60.0, 80.5, 1000.0}, Instrument::QUAD,
                         {200, 0.01, 1.0});
    ...
    SUBCASE(
        ...
        auto mesh = RegularMesh({4, 4}, {0.0, 60.0, 80.5, 1000.0}, Instrument::QUAD,
                         {200, 0.01, 1.0});
...
TEST_CASE("Real world dimensions fetching via RegularMesh::mz and RegularMesh::rt") {
    SUBCASE("An empty mesh should always return std::nullopt") {
        auto mesh = RegularMesh({0, 0}, {0.0, 75.0, 200.0, 800.0}, Instrument::QUAD,
                         {200, 0.01, 1.0});
    ...
    SUBCASE("Proper interpolation if inside bounds or std::nullopt") {
        auto mesh = RegularMesh({4, 4}, {0.0, 75.0, 200.0, 800.0}, Instrument::QUAD,
                         {200, 0.01, 1.0});
...
TEST_CASE("Getters for mDimensions and mBounds") {
    ...
    SUBCASE("With a non empty mesh") {
        auto mesh = RegularMesh({4, 10}, {0.0, 75.0, 200.0, 800.0}, Instrument::QUAD,
                         {200, 0.01, 1.0});
        auto dimensions = mesh.dim();
        auto bounds = mesh.bounds();
